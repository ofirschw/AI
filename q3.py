...
def IG(data, feature, output_name): #  this function will calculate the information gain
    ...
    weight = 0
    for i in range(len(vals)):
        weight = weight + np.sum([(counts[i] / np.sum(counts)) * calc_entropy(data.where(data[feature] == vals[i])[output_name])])
    sum_entropy = calc_entropy(data[output_name])
    information_gain = sum_entropy - weight
    return information_gain


def createDecisionTree(data, originaldata, features, output_name,iteration, parents): #create the tree for answer
    if len(data) == 0:
        ...
        for value in np.unique(data[best_feature]):  #creating next tree for each feature
            index = value
            next_data = data.where(data[best_feature] == value)
...
def is_busy(row_input):
    ...
    temp_row = temp_row.drop('Date', axis=1) #remove Date column
    temp_row = temp_row.drop('year', axis=1)#remove year column
    ...
    predicted_result = testDataFrame(dataframebusy, tree)
    return int(predicted_result['predict'][0])
#############################################is_busy function################################################################
#############################################tree_error function################################################################
def tree_error(k):
    print("------------------------------------")
